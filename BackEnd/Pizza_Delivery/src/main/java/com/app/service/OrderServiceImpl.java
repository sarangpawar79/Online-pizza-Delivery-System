...
//		
//	}
		Payment payment = createPayment(total, deliveryCharges, paymentMode, newOrder);
		paymentRepo.save(payment);

		saveOrderDetailsForCartItems(cartItems, newOrder);
		...
		// Helper method to create a new Payment object
		private Payment createPayment(double total, int deliveryCharges, String paymentMode, FoodOrder newOrder) {
		    PaymentStatus paymentStatus = paymentMode.equals("COD") ? PaymentStatus.PENDING : PaymentStatus.COMPLETED;
		    PaymentModes paymentModes = PaymentModes.valueOf(paymentMode);
		    double paymentAmount = total + deliveryCharges;

		    return new Payment(paymentAmount, paymentStatus, LocalDateTime.now(), paymentModes, newOrder);
		}

		// Helper method to save OrderDetails for each Cart item
		private void saveOrderDetailsForCartItems(List<Cart> cartItems, FoodOrder newOrder) {
		    for (Cart item : cartItems) {
		        int quantity = item.getQuantity();
		        double itemTotal = item.getSelectedMenu().getPrice() * quantity;
		        Menu selectedMenu = item.getSelectedMenu();

		        OrderDetails orderDetails = new OrderDetails(quantity, itemTotal, newOrder, selectedMenu);
		        orderDetailsRepo.save(orderDetails);
		    }
		    
		}

		// Helper method to delete Cart items
		private void deleteCartItems(List<Cart> cartItems) {
		    cartRepo.deleteAll(cartItems);
		}
